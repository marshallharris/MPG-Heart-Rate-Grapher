...
void LcdShift(PixelBlockType* eShiftArea_, u16 u16PixelsToShift_, LcdShiftType eDirection_)
{
  ...
  u8 u8CurrentBitMask;
  u16 u16startBitGroup;
  u8 u8RowIterations;
  u16 u16ColumnIterations;
  u16 u16TotalBitGroups;
  u16 u16CopyPixel = 0x0000;
  u16 u16newColumnStart;
  u16 u16newStartBitGroup;
  u16 u16newTotalBitGroups;
  int u16currentBitGroup;    
  u16 u16newCurrentBitGroup;
  
  if(eDirection_ == LCD_SHIFT_RIGHT){
  PixelBlockType tempBlock = *eShiftArea_;
     
      u16ColumnIterations = eShiftArea_->u16ColumnSize;
      u8RowIterations = eShiftArea_->u16RowSize;
      
      u16startBitGroup = (eShiftArea_->u16ColumnStart / 8);
      double temp = ((eShiftArea_->u16ColumnStart  + eShiftArea_->u16ColumnSize)/8.0 - u16startBitGroup) + 0.99;
      u16TotalBitGroups = (u16)temp;
      u16currentBitGroup  = u16startBitGroup + u16TotalBitGroups - 1;  
      
      
      while( u16currentBitGroup >= u16startBitGroup){
      
      for(u16 i = eShiftArea_->u16RowStart; i < (eShiftArea_->u16RowStart + u8RowIterations); i++)
      {
        u16CopyPixel = 0;
      
        
       // for( u16 j = 0; j < u16TotalBitGroups; j++){
          u16CopyPixel |= G_aau8LcdRamImage[i][u16currentBitGroup]; //<< 8*j;
        //}
        
        //triming what is not supposed to be shifted off the end of the group
          if(  u16currentBitGroup == u16startBitGroup){
            u16CopyPixel &= 0xFFFF << (eShiftArea_->u16ColumnStart % 8);
            u16newColumnStart = (eShiftArea_->u16ColumnStart + u16PixelsToShift_);
            u16newStartBitGroup = (u16newColumnStart / 8);
            temp = ((u16newColumnStart  + eShiftArea_->u16ColumnSize)/8.0 - u16newStartBitGroup) + 0.99;
            u16newTotalBitGroups = (u16)temp;
         
          }
          else if(u16currentBitGroup == u16startBitGroup + u16TotalBitGroups - 1){
            u16CopyPixel &= 0x00FF >> 8*u16TotalBitGroups-(eShiftArea_->u16ColumnStart + eShiftArea_->u16ColumnSize);
            u16newColumnStart = (u16currentBitGroup*8 + u16PixelsToShift_);
            u16newStartBitGroup = (u16newColumnStart / 8);
          }
          else{
            u16newColumnStart = (u16currentBitGroup*8 + u16PixelsToShift_);
        ...
        u16CopyPixel = (u16CopyPixel) << (u16PixelsToShift_ % 8);
        u8 u8Lower = 0x00FF & u16CopyPixel;
        u8 u8Upper = u16CopyPixel >> 8;
        int k = 0;
        u8 topBit = u8Upper;
        while(topBit != 0x0){
          topBit =  topBit >> 1;
        ...
        u8 bitsToClear = 0xFF << k;
        G_aau8LcdRamImage[i][u16newStartBitGroup + 1] &= bitsToClear;
        //for( u16 k = 0; k < u16newTotalBitGroups; k++){
        
        
          G_aau8LcdRamImage[i][u16newStartBitGroup] = u8Lower;
          
         
          G_aau8LcdRamImage[i][u16newStartBitGroup + 1] |= u8Upper;         
       // }
      }  
      
      
   
   tempBlock.u16ColumnStart += u16PixelsToShift_;
   LcdUpdateScreenRefreshArea(&tempBlock);   
   u16currentBitGroup--;
   }
   
   PixelBlockType tempBlock2 = *eShiftArea_;
   tempBlock2.u16ColumnSize = u16PixelsToShift_;
   LcdClearPixels(&tempBlock2);
   *eShiftArea_ = tempBlock;
  }
  
  /*
  if(eDirection_ == LCD_SHIFT_LEFT){
      PixelBlockType tempBlock = *eShiftArea_;
      ...
      double temp = ((eShiftArea_->u16ColumnStart  + eShiftArea_->u16ColumnSize)/8.0 - u16startBitGroup) + 0.99;
      u16TotalBitGroups = (u16)temp;
      u16currentBitGroup  = u16startBitGroup;  
      
      
      while( u16currentBitGroup <=  u16startBitGroup + u16TotalBitGroups - 1){
      ...
      for(u16 i = eShiftArea_->u16RowStart; i < (eShiftArea_->u16RowStart + u8RowIterations); i++)
      {
       ...
       // for( u16 j = 0; j < u16TotalBitGroups; j++){
          u16CopyPixel |= G_aau8LcdRamImage[i][u16currentBitGroup] << 8; //<< 8*j;
        //}
        
        //triming what is not supposed to be shifted off the end of the group
          if(  u16currentBitGroup == u16startBitGroup){
            u16CopyPixel &= 0xFF00 << (eShiftArea_->u16ColumnStart % 8);
            u16newColumnStart = (eShiftArea_->u16ColumnStart - u16PixelsToShift_);
            u16newStartBitGroup = (u16newColumnStart / 8);
            temp = ((u16newColumnStart  + eShiftArea_->u16ColumnSize)/8.0 - u16newStartBitGroup) + 0.99;
            u16newTotalBitGroups = (u16)temp;
            ...
            u16CopyPixel &= 0xFF00 >> 8*u16TotalBitGroups-(eShiftArea_->u16ColumnStart + eShiftArea_->u16ColumnSize);
            u16newColumnStart = (u16currentBitGroup*8 - u16PixelsToShift_);
            u16newStartBitGroup = (u16newColumnStart / 8);
          }
          else{
          ...
          if(u16newStartBitGroup == u16currentBitGroup){
            u16CopyPixel = (u16CopyPixel) >> (u16PixelsToShift_ % 8);
            u8 u8Lower = u16CopyPixel >> 8;
            int k = 0;
            u8 topBit = u8Lower;
            while(topBit != 0x0){
              topBit =  topBit << 1;
            ...
            u8 bitsToClear = 0xFF >> k;
            G_aau8LcdRamImage[i][u16newStartBitGroup ] &= bitsToClear;
            G_aau8LcdRamImage[i][u16newStartBitGroup] |= u8Lower;
          }
          else{
        u16CopyPixel = (u16CopyPixel) >> (u16PixelsToShift_ % 8);
   ...
   PixelBlockType tempBlock2 = *eShiftArea_;
   tempBlock2.u16ColumnStart += tempBlock2.u16ColumnSize;
   tempBlock2.u16ColumnSize = u16PixelsToShift_;
   LcdClearPixels(&tempBlock2);
   *eShiftArea_ = tempBlock;
 ...
 if(eDirection_ == LCD_SHIFT_LEFT){   
    ...
    while( u16currentBitGroup <=  u16startBitGroup + u16TotalBitGroups - 1){
      for(u16 i = eShiftArea_->u16RowStart; i < (eShiftArea_->u16RowStart + u8RowIterations); i++)
      {
        u16CopyPixel = G_aau8LcdRamImage[i][u16currentBitGroup];
        
        if( u16currentBitGroup == u16startBitGroup){
            u16CopyPixel &= 0xFF << (eShiftArea_->u16ColumnStart % 8);
            u16newColumnStart = (eShiftArea_->u16ColumnStart - u16PixelsToShift_);
            u16newStartBitGroup = (u16newColumnStart / 8) ;
            temp = ((u16newColumnStart  + eShiftArea_->u16ColumnSize)/8.0 - u16newStartBitGroup) + 0.99;
            u16newTotalBitGroups = (u16)temp;
          ...
          u16CopyPixel &= 0xFF >> 8*u16TotalBitGroups-(eShiftArea_->u16ColumnStart + eShiftArea_->u16ColumnSize);
          u16newStartBitGroup = (u16currentBitGroup - u16PixelsToShift_/8); 
          u16CopyPixel = (u16CopyPixel) >> (u16PixelsToShift_ % 8);
          u8 u8Lower = 0x00FF & u16CopyPixel;
          u8 topBit = u8Lower;
          int k = (eShiftArea_->u16ColumnStart+eShiftArea_->u16ColumnSize)%8;
        ...
        if( u16currentBitGroup > u16startBitGroup && u16currentBitGroup < u16startBitGroup + u16TotalBitGroups - 1){
          u16newStartBitGroup = (u16currentBitGroup - u16PixelsToShift_/8);
        }
        
        
        // bring down bits from the next group
        u8 bitShifter = 0xFF;
        bitShifter = bitShifter << u16PixelsToShift_;
        //isolate the bits
        bitShifter = G_aau8LcdRamImage[i][u16currentBitGroup + 1] & (~bitShifter);
        
        //circular shift them
        //bitShifter = (bitShifter << u16PixelsToShift_) | (bitShifter >> (8 - u16PixelsToShift_));
        bitShifter =  bitShifter << (8 - u16PixelsToShift_);
        
        u16CopyPixel = (u16CopyPixel) >> (u16PixelsToShift_ % 8);
        u16CopyPixel |= bitShifter;
        u8 u8Lower = 0x00FF & u16CopyPixel;
        u8 topBit = u8Lower;
        int k =0;
        while(topBit != 0x00){
      ...
      tempBlock.u16ColumnStart = eShiftArea_->u16ColumnStart - u16PixelsToShift_;
      LcdUpdateScreenRefreshArea(&tempBlock);   
      u16currentBitGroup++;
      
    }
   PixelBlockType tempBlock2 = *eShiftArea_;
   tempBlock2.u16ColumnStart = tempBlock.u16ColumnStart + tempBlock.u16ColumnSize;
   tempBlock2.u16ColumnSize = u16PixelsToShift_;
   LcdClearPixels(&tempBlock2);
   *eShiftArea_ = tempBlock;
 ...
 if(eDirection_ == LCD_SHIFT_LEFT){
  PixelBlockType tempBlock = *eShiftArea_;
  u8 u8RowIterations = eShiftArea_->u16RowSize;
  for(u16 i = eShiftArea_->u16RowStart; i < (eShiftArea_->u16RowStart + u8RowIterations); i++)
  {
    u8 currentColumn = eShiftArea_ -> u16ColumnStart;
    u8 newColumn = (eShiftArea_ -> u16ColumnStart - u16PixelsToShift_);
    u8 currentPixel = 0;
    u8 newPixel = 0;
    
    while(currentColumn < eShiftArea_ -> u16ColumnStart + eShiftArea_ ->u16ColumnSize){
      u8 currentGroup = G_aau8LcdRamImage[i][currentColumn/8];
      u8 correctBit = 0x01 << (currentColumn % 8);
      currentPixel = currentGroup & correctBit;
      //shift down
      currentPixel = currentPixel >> (currentColumn % 8);
      newPixel =  currentPixel << (newColumn%8);
      u8 clearBit = 0x1 << (newColumn%8);
      // clear the specified bit
      G_aau8LcdRamImage[i][newColumn/8] &= (~clearBit);
      
      //put in the pixel in it's new place
      G_aau8LcdRamImage[i][newColumn/8] |= newPixel;
      currentColumn++;
      newColumn++;
    }
  }
